...
- 通常の開発作業と push は `main` を基準にし、`codex/*` ブランチは大きい試作や隔離したい作業だけで使う
...
- 通常リリースは `MAJOR.MINOR.PATCH`（例：`1.6.0`）を使う
- 機能追加を含むときは `MINOR` を上げる
- 修正のみの時には `PATCH` を上げてリリースする。（例：`1.5.1` -> `1.5.2`）
...
- GitHub Issues の更新要否を確認し、必要なら Issue / ラベル / 状態を反映すること
...
- ライトボックスの設計方針は `docs/dev/specs/lightbox-redesign-v3.md` に従うこと（v2 は deprecated）
