...
## v1.2 追記
- プロファイル単位で `自動実行` 設定を持ち、`手動スキャン後 / 起動時スキャン後 / 監視再スキャン後` を個別に ON/OFF できるようにする。
- ルール単位でも `スキャン完了後にも自動実行する` を持ち、プロファイル設定とルール設定の両方が有効なときだけ実行する。
- 自動実行時はスキャンしたルートフォルダ配下だけを対象にし、別フォルダの一致ファイルまでは巻き込まない。
- 自動実行は既存の Dry Run 判定を流用し、競合や不正な名前は従来どおりスキップする。
- 自動整理の適用結果は専用履歴として保持し、`どのルールが / どのファイルへ / どこへ適用されたか` を後から確認できるようにする。
- ロールバックは `完全な元に戻す機能` ではなく補助扱いとし、現在位置不一致・元フォルダ消失・元位置競合がある項目はスキップまたは競合で止める。
- ロールバックも `Dry Run -> Apply` の順に限定し、即時巻き戻しは入れない。
