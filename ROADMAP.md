...
- タグカテゴリ表示優先順位強化（進行中）: FileCard の要約タグ表示にカテゴリ分散ロジックを導入しつつ、カテゴリ表示順を基準にした並びを反映（長いタグ名はチップ内で省略表示して崩れ抑制）。設定画面から FileCard 要約タグの並びを `カテゴリ分散` / `厳密順（カテゴリ順→タグ順）` で切替可能にした。タグ管理モーダルのタグ管理タブを整理しつつ、表示優先順位の基準（`sortOrder` / カテゴリ分散）は情報ボタンのポップアップで確認できるよう見える化し、タグ一覧でドラッグによる表示優先順変更を可能にした。あわせてタグアイコン名の入力補助（候補/クイック選択）を追加し、左サイドバーのタグ検索フィルター / 右サイドバー / Lightbox のタグ表示にも表示優先順ベースの並びを反映。次は他画面への適用要否を確認
