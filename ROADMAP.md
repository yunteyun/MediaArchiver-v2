...
**優先度**: 🔴 最優先  
**目的**: Phase 20着手前の緊急バグ修正

- [ ] EventEmitter メモリリーク警告（onRequestMove）
- [ ] 外部アプリカウント表示の不具合（動画・音声ファイル）
- [ ] 複数選択時の削除/移動の不具合
- [ ] ファイル移動後の再スキャン問題
- [ ] 重複ファイル検索の再実行不可
- [ ] 書庫ファイルの孤立サムネイル問題
...
**優先度**: 🟠 高  
**目的**: Lightbox全面改良とメディア再生機能の拡張

- [ ] Lightbox全面改良（レイアウト、操作性、表示干渉）
- [ ] プログレスバー表示干渉修正
- [ ] 動画途中再生（ランダム位置）
- [ ] 音声ファイルのプログレスバー改善
- [ ] 既存モード拡張（再生速度調整）
