...
### Phase 23: 右サイドパネル 🗂️
**目的**: 選択ファイルの詳細情報を常時表示し、グリッド（探索）・右パネル（整理）・Lightbox（鑑賞）の三層構造を確立する
**参考**: [右パネル実装についてのメモ.md](右パネル実装についてのメモ.md)

- [ ] `useUIStore` に `isRightPanelOpen` / `rightPanelWidthMode` / `autoPreviewEnabled` を追加
- [ ] `RightPanel` コンポーネント作成（固定幅 360px）
- [ ] プレビューエリア（高さ固定 300px、動画 muted/controls なし、クリックで Lightbox 遷移）
- [ ] 基本情報エリア（ファイル名・解像度・再生時間・サイズ・作成日）
- [ ] タグ表示エリア（カラー表示、折り返しレイアウト）
- [ ] タグ編集導線（「編集」ボタン → 既存モーダル起動）
- [ ] グリッドホバープレビューとの排他制御
- [ ] ヘッダーにパネル開閉トグルボタン追加
...
### Phase 24: サムネイル軽量化 🗜️
**目的**: サムネイル容量を削減し、ファイル数増加時のストレージ圧迫を防ぐ
**現状**: 1000ファイルで約 1GB、動画プレビューフレームが PNG で 500KB〜1MB

- [ ] 方針決定（WebP 変換 / 品質調整 / 解像度縮小）
- [ ] 静止画サムネイルを WebP 化（品質 80 程度）
- [ ] 動画プレビューフレームを WebP 化（whitebrowser 方式参考: 240×180×5枚 → 1枚結合、目標 100KB 以下）
- [ ] 書庫内画像サムネイルのリサイズ処理追加
- [ ] 既存サムネイルの一括再生成コマンド実装
- [ ] サムネイル容量レポート（統計画面に追加）
...
### Phase 25: 保存場所カスタマイズ ⚙️
**目的**: サムネイル・DB の保存先を AppData 以外に変更可能にし、ユーザーの利便性を向上させる

- [ ] 設定画面にサムネイル保存先パス設定を追加
- [ ] 設定画面に DB 保存先パス設定を追加
- [ ] パス変更時の既存ファイル移行処理
- [ ] 起動時に保存先パスの存在確認・フォールバック処理

---

### Phase 26: タグ構造刷新 🏷️
**目的**: タグ 100〜300 件規模に対応する二層カテゴリ構造と、評価軸の物理的分離を実現する
**参考**: [タグ構造の刷新に関するメモ.md](タグ構造の刷新に関するメモ.md)

- [ ] DB マイグレーション設計（`tag_categories` テーブル追加、`tags.category_id` 追加）
- [ ] 評価軸テーブル設計（`rating_axes` / `file_ratings`）
- [ ] タグ管理画面を左右ペイン構造に刷新（左: カテゴリ一覧、右: タグ一覧）
- [ ] 評価軸管理画面を独立実装
- [ ] Lightbox タグセクション・評価セクションの分離表示
- [ ] 既存タグデータのマイグレーション
- [ ] カテゴリ・評価軸を使った詳細検索 UI
