...
- サムネイル保存管理 Step 2（追加）: 書庫プレビューフレームも優先 WebP 化（256px基準に縮小）し、容量削減を進めた（変換失敗時は元形式フォールバック）
- サムネイル保存管理 Step 2（追加）: 書庫プレビューフレームの最終保存先を `archive-preview` 配下へ統一し、一時ファイル依存を減らして管理しやすくした
