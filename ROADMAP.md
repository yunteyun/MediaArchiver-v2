...
**Phase 13: FileCard 基礎設計** 🏗️ (In Progress)

> [!NOTE]
> Phase 14 の表示モードシステムに向けた、FileCard コンポーネントの再設計とリファクタリング。
> **「設計の保険」を適用中**（アーキテクチャレビュー反映）

**実装タスク**:
1. [x] **画像ファイルの扱い**
   - [x] マイグレーション: `is_animated` カラム追加
   - [x] スキャナー: GIF/WebPアニメーション判定
2. [ ] **FileCard 表示情報（デフォルト構成）**
   - [ ] フォルダ名表示（`utils/path.ts`）
   - [ ] ファイル名下寄せレイアウト
3. [ ] **情報レイアウト方針**
   - [ ] 縦積み構造への変更
   - [ ] FileCardMeta境界の明確化（TODOコメント）
4. [ ] **タグ表示ルール**
   - [ ] `utils/tag.ts` による整形ロジック分離
   - [ ] 定数化された最大表示数
5. [ ] **配色ルール（デザイントークン化）**
   - [x] CSS変数によるバッジ色定義
...
> [!NOTE]
> Phase 13 で確立した基盤の上に、多様な表示モードを実装します。

**実装予定機能**:
1. 標準モード（現在の正方形カード）
2. 漫画モード（縦長カード、縦長サムネイル）
3. 動画モード（横長カード、横長サムネイル）
4. WhiteBrowser風モード（横長レイアウト、情報量多め）
5. カスタムモード（ユーザー定義）
