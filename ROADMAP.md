...
| エクスポート/インポート改善（大規模データ時） | 列選択・選択ファイル限定・大量データ時のUX/性能調整 | 中 | 低〜中 | 基本機能は実装済み。データ増加時に再調整候補 |
...
- インポート/エクスポート（バックアップタブ集約）: 一覧エクスポート（CSV / HTML）を設定画面バックアップタブへ集約し、`プロファイル全体` / `現在選択フォルダ全体` を選んで出力可能にした（HTML は閲覧用、インポート対応は CSV のみ）。あわせて「このアプリ形式CSV」インポート（Dry Run付き）と、旧アプリCSV（Shift_JIS/可変列）互換インポート（タグ + 星評価 + `コメント１`→メモ、Dry Run付き）を追加。今後データ量増加時は列選択・選択ファイル限定・UX/性能の再調整を検討
