...
| 29 | 実行環境分離とプロファイル整合 | `profiles.db` を保存先スコープに統一、`appdata` の dev/release 自動分離で開発版とリリース版のプロファイル混在を解消、ログ保存先も `logs/dev`・`logs/release` に分離、ZIP更新用 `update.bat` を同梱 | 2026-02 |
