...
- [x] 10-2: バックアップ・リストア機能 ✅
  - データベースの手動バックアップ（VACUUM INTO 使用）
  - ディスク容量事前チェック（DBサイズの1.5倍）
  - 安全なリストアフロー（db.close() → copy → app.relaunch()）
  - バックアップ履歴管理（ファイル名からタイムスタンプをパース）
  - 世代数制限による自動削除
  - **効果**: データ損失リスクの低減

- [x] 10-3: パフォーマンス最適化（第2弾） ✅
  - タグキャッシュの一括取得（N回→1回のIPC呼び出しに削減）
  - サムネイル生成の同時実行数制限（p-limit使用）
  - **効果**: 大規模ライブラリでの快適な動作

- [x] 10-4: ファイルパス検証とエラーハンドリング強化 ✅
  - パス長の検証（Windows 260文字制限対応）
  - 無効文字のチェック
  - アクセス権限エラーの適切な処理（EPERM/EACCES/ENOENT/ENAMETOOLONG）
  - **効果**: クラッシュリスクの低減
...
- [x] 11-1: カテゴリ別統計表示 ✅
  ...
  - タグ別棒グラフ（上位20件）
  - フォルダ別棒グラフ
  - 月別登録推移（折れ線グラフ、過去12ヶ月）
  - 未整理ファイル率（タグあり/なし、ドーナツチャート）
...
- [x] 11-2: 追加統計機能 ✅
  ...
  - 拡張子ランキング（Top 20、グリッド表示）
  - 解像度分布（4K/FHD/HD/SD、円グラフ）
  - metadata JSON解析による解像度分類

- [x] 11-3: アクティビティログ ✅
  - ファイル追加・削除履歴（Fire-and-Forget方式）
  - タグ付け履歴（追加・削除）
  - タイムライン形式表示（日付グルーピング、相対時間）
