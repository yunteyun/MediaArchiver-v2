...
- [ ] 12-17: ファイル削除ダイアログのUI改善
  - 現状: 3ボタン（ゴミ箱に移動 / 完全に削除 / キャンセル）
  - 改善案: 2ボタン + チェックボックス方式
    - デフォルト: ゴミ箱に移動
    - チェックを外すと完全削除
  - よりスムーズなUX
...
- [ ] 12-18: サムネイル再作成時のスクラブサムネイル確認
  - 再作成前にスクラブサムネイル（preview_frames）の存在を確認
  - 不足している場合のみ再生成（負荷軽減）
...
**Phase 13 で「やらないこと」**（Phase 14 以降へ移管）:
- [ ] サムネイルの実際の再生（hover 再生含む）
- [ ] スクラブ操作の拡張
- [ ] 配色の最終確定（色値の決定）
- [ ] 表示モードシステム（標準/漫画/動画/WhiteBrowser風）
- [ ] サムネイルアスペクト比対応
- [ ] カード幅の動的調整
