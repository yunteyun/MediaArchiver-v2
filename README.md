...
- 不具合修正のみの場合は同一ベース版に `d` を付ける（例: `1.7.0d1`, `1.7.0d2`）
- `PATCH` は「修正版をまとめた安定区切り版」を出すときに上げる（例: `1.7.0d3` -> `1.7.1`）
- `d` 番号は同一ベース版内で連番にする
- 次の通常リリース時は `d` をリセットして `MAJOR.MINOR.PATCH` に戻す
- 出力ZIP名は electron-builder の都合で `1.7.0-d2` 形式になる
