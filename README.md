...
| オプション | 説明 |
|-----------|------|
| `--profile <名前>` | 起動時に開くプロファイルを指定する |

**例（Windowsショートカット）:**
ショートカットのリンク先末尾に `--profile "仕事用"` を追記すると、そのショートカットから起動した場合に「仕事用」プロファイルが開く。

指定したプロファイル名が存在しない場合は、前回使用していたプロファイルで起動する。
...
- 使い方と更新手順: `docs/user/アプリ使用メモ.md`
- 保存場所とログ: `docs/user/保存場所とログ.md`
