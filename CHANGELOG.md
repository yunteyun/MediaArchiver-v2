...
- リリース版アイコンをタスクバー視認性重視で再調整し、背景グラデーションを弱め、フォルダ上部タブ色を2色へ整理した。
- ユーザー提供のアイコンセットへ差し替え、Windowsビルドは `.ico` を直接参照してタスクバー上の表示を安定化した。
- 開発用アイコンを色反転ベースへ変更し、`DEV` バッジを重ねてリリース版と視覚的に判別しやすくした。
- ファイルカード表示統一の仕上げとして、簡易情報欄のタグ要約件数を表示モード定義参照へ一本化し、モード別プリセットとの管理差分を解消した（表示への変更なし）。
- ファイルカード表示統一の仕上げとして、グリッド側に残っていたモード別カード伸長量も表示モード定義へ統合し、表示調整値の管理を一元化した（表示への変更なし）。
- 表示モード名を見直し、`漫画` を `縦型`、`詳細表示（漫画）` を `詳細表示（縦）` へ変更して用途を判別しやすくした。
- バージョン命名規則を整理し、通常リリース（`MAJOR.MINOR.PATCH`）と修正版（`d` 連番）の使い分けを README/運用ルールへ明文化した（表示への変更なし）。
- バージョン命名規則に `PATCH` の扱いを追加し、同一ベース版の修正版（`d`）をまとめた安定区切りで上げる運用を明文化した（表示への変更なし）。
