...
- 書庫ファイルのファイルカードでホバー時のパラパラ表示に対応し、一覧上でも書庫内画像の雰囲気を確認しやすくした。
- 一覧プレビュー設定で書庫カードのホバー動作を動画と分離し、書庫だけ `オフ / 自動パラパラ` を切り替えられるようにした。
- 書庫カードのパラパラ表示で取得枚数と再生間隔を見直し、`遅い` 設定時でも慌ただしく見えにくいテンポへ調整した。
- 一覧プレビュー設定のパラパラ速度を動画用と書庫用で分離し、書庫だけ遅めに調整するなど運用に合わせて個別設定できるようにした。
