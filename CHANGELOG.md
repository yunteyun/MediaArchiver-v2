...
- 右パネルのタグ欄は編集を押すとカテゴリ選択がそのまま見える導線へ見直し、カテゴリを選ぶとタグ候補が下優先のポップアップで開いて `Esc` や外側クリックではまずタグ候補を閉じる形で編集できるようにした。
- 動画の `見どころ` は、再開位置セクションから開くポップオーバーで現在位置の保存や保存済み場面からの再生をできるようにし、保存後に一言メモを編集できるようにした。
- 動画を中央ビューアで開いたまま右パネルから `今の場面を表紙にする` を使えるようにし、保存済みの `見どころ` からも表紙固定できるようにした。あわせて表紙操作は小さいアイコンボタンへ整理した。
- 書庫ファイルも右パネルのプレビュー画像から `このページを表紙にする` を選べるようにし、取得済みプレビューの中から代表サムネイルを固定できるようにした。

### Changed
- 右パネルの再開位置セクションは、保存時刻と操作ボタンの見え方を詰めて縦に場所を取りすぎない表示へ調整した。
- 見どころポップオーバーは、各項目の操作を上下に整理してボタンを詰め込みすぎない表示へ調整した。
- 見どころポップオーバーは、メモ編集を `保存 / キャンセル` 中心の流れへ整理し、追加直後の反応・現在位置に近い見どころの分かりやすさ・メモなし項目の表示密度を調整した。
- 見どころは近い時刻の重複保存を避けつつ、表示順を `時刻順 / 新しい順` で切り替えられるようにした。
- 右パネルは `再開位置` を外して情報表示へ寄せ、動画の再開や見どころ整理はライトボックス側へ集約した。あわせて右パネルの `メモ` に見どころメモの要約を統合した。
- 中央ビューア左上の `再開 / 見どころ` は、背景をしっかり持つ小パネルに見直し、見どころ追加時にその場でメモを添えられるようにした。
- 中央ビューア左上の `見どころ一覧` は同じ小パネル内で開く形にまとめ、一覧内の `ここから開く` をやめて再生時間部分を押すとその場面へ飛べるようにした。
- 右パネルの `メモ` 内にある見どころ一覧も開閉式へ整理し、時刻とメモを押すとその場面へ飛べるようにした。
- 右パネルの `メモ` は、見どころ一覧を常時見える形へ戻し、ファイルメモ入力欄だけをクリックで開く方式にして縦の圧迫感を減らした。
- 右パネルのファイルメモは、クリックした枠がそのまま入力欄へ切り替わる形に見直し、編集の流れを自然にした。
