...
- **Phase 14: FileCardレイアウト改善** (2026-02-10)
  - **Standardモード刷新**: 3行レイアウト（ファイル名 + フォルダ名 + サイズ＆タグ）を採用し、ファイル名を最優先表示
  - **Compactモード追加**: 2行レイアウト（ファイル名 + サイズ＆タグ）、タグ最大2個表示の省スペースモード
  - **タグ展開機能**: `+N` ボタンで全タグを展開・`▲` ボタンで折りたたみ（Lightbox起動防止対応）
  - **表示調整**:
    - カード幅拡大（Standard: 200px → 300px）
    - サイズ表記の位置・フォントをタイトルと統一
    - グリッドレイアウトの高さ固定化による表示崩れ防止

- **サムネイル再生成の進捗表示機能** (2026-02-09)
  - ファイル右クリックからのサムネイル再生成時に進捗ウィンドウを表示
  - トースト通知（「サムネイルを再生成しました」）を追加
  - エラー時の通知も対応

- **ffmpeg スレッド制限によるコイル鳴き軽減** (2026-02-09)
  - プレビューフレーム生成時に `-threads 1` を指定
  - 瞬間的なCPU負荷スパイクを軽減し、多くの動画でコイル鳴きを抑制
  - 一部の重い動画（HEVC、4K等）では依然として発生する可能性あり
  - 完全解決はWorker化（Phase 13以降）で対応予定
  - 詳細は `CONVENTIONS.md` の「既知の技術的制約と対策」を参照

### Fixed
- **Phase 12-18 バグ修正: サムネイル再作成時のプレビューフレーム生成** (2026-02-09 15:45)
  - 既存フレーム数チェックを削除（ユーザーが明示的に再作成を選択した場合は常に再生成）
  - ログ出力の改善（再生成開始・成功・失敗を明確に表示）
  - UX改善: 「サムネイル再作成」が期待通りに動作するように修正

- **スキャナー最適化: プレビューフレーム設定0の場合の認識改善** (2026-02-09 15:52)
  - 設定値が0の場合、プレビューフレームがなくても動画ファイルを「完了」とみなすように修正
  - 不要な再スキャンを防止し、パフォーマンスを改善

- **スキャナー改善: プレビューフレームファイルの存在確認** (2026-02-09 16:15)
  - DBに記録されているだけでなく、実際のファイルが存在するかを確認するように修正
  - 孤立したDBレコード（ファイルが削除されている）を検出し、再生成するように改善
  - スクラブ時のエラーを防止
