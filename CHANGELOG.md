...
- `docs/dev/operations/運用前チェックリスト.md` を追加し、本格運用へ入る前に確認したいバックアップ、スキャン、更新導線の確認項目を整理した（表示への変更なし）。
...
### Changed
- バックアップ設定を再起動後も保持するようにし、保存先、保持世代数、自動バックアップ間隔を設定画面から継続利用できるようにした。
- バックアップタブに保存先設定、自動バックアップ、復元用の履歴一覧を追加し、運用前後の保守作業を設定画面から完結しやすくした。
