...
- 重複チェック画面に `類似候補` モードを追加し、完全一致ではないが名前が同じ・連番違いのファイルを候補として洗い出せるようにした。
...
- 書庫ファイルのファイルカードでホバー時のコマ送り表示に対応し、一覧上でも書庫内画像の雰囲気を確認しやすくした。
- 一覧プレビュー設定で書庫カードのホバー動作を動画と分離し、書庫だけ `オフ / コマ送り` を切り替えられるようにした。
- 書庫カードのコマ送り表示で取得枚数と再生間隔を見直し、`遅い` 設定時でも慌ただしく見えにくいテンポへ調整した。
- 一覧プレビュー設定のコマ送り速度を動画用と書庫用で分離し、書庫だけ遅めに調整するなど運用に合わせて個別設定できるようにした。
- 一覧プレビュー設定の表記を `自動パラパラ` から `コマ送り` へ整理し、動画再生との違いを誤解しにくくした。
- 一覧プレビュー設定の項目順を整理し、動画ホバー動作の直下で動画の速度設定を確認できるようにした。
- 書庫カードでコマ送り中の補助表示を追加し、画像書庫バッジも出して音声書庫と画像書庫を一覧で見分けやすくした。
- 書庫カードのコマ送り補助表示が拡大ボタンと重なりにくい位置へ調整し、表記も `3/8` のような最小限表示へ整理した。
- 書庫カードのコマ送り補助表示を枚数バッジの横へ移し、同じサイズ感でまとめて見やすくした。
- 重複チェック画面に全体ヘッダーの一括戦略ボタン、グループ単位の選択解除、各ファイルの「このファイルを残す」導線を追加し、残したい側を見ながら一括整理しやすくした。
- 重複チェック画面の選択文言を「何を残すか」ではなく「何を削除候補にするか」中心へ整理し、削除対象を選ぶ導線を分かりやすくした。
- 重複チェック画面で保存先フォルダを主表示へ寄せ、別フォルダに分散した重複はフォルダごとに色分けして見落としにくくした。
- 重複チェック画面の各ファイル行にサイズバッジを追加し、グループ見出しを見返さなくても容量感を確認しながら残す対象を判断しやすくした。
- 重複チェック画面でファイルサイズがグループ基準と異なる場合はサイズバッジと注意文を警告色にし、想定外のサイズ差に気づきやすくした。
- 重複チェック画面の見出しと集計表示を検索モードごとに切り替え、完全一致と類似候補で意味の違う件数やサイズ帯を誤解しにくくした。
- 重複チェック画面の日時表示を強調し、各行の最新候補 / 最古候補ラベルとグループ全体の日時幅を見ながら残す対象を判断しやすくした。
- 登録フォルダ追加時とフォルダ別スキャン設定で、除外する子フォルダを相対パス指定できるようにし、大きな親フォルダ配下から必要範囲だけを継続運用しやすくした。
- 右サイドパネルの動画プレビュー方式に `停止` を追加し、静止サムネイルだけを見たい運用でも誤再生しにくくした。
- 右サイドパネルの動画プレビュー操作を見出し行のアイコントグルへ集約し、再生/停止を追加したうえでプレビュークリックは拡大表示へ戻した。
...
- スキャン実行中にプロファイルを切り替えられて別プロファイルへ登録が混入する問題を修正し、スキャン中は切替を止めて誤登録を防ぐようにした。
- スキャンキャンセル後に次のスキャン進捗表示が崩れることがある問題を修正し、古い進捗イベントが新しいスキャン表示へ割り込まないようにした。
- ファイル移動ダイアログで登録フォルダや既存の子フォルダが表示されない問題を修正し、登録済みルート配下の既知フォルダへ直接移動できるようにした。
- 重複チェック画面で削除候補と残す対象の判別がしづらい問題を修正し、グループごとの選択件数と各ファイルの状態が分かるようにした。
- 重複チェック画面の全件選択で描画負荷が跳ね上がる問題を修正し、大量選択時でも操作不能になりにくいようにした。
- 重複チェック画面の一括マーキングが重くなりすぎる問題を修正し、負荷対策として先頭100グループまでに上限を設けた。
- 重複チェック画面の全件選択ボタンを廃止し、意図せず全件を削除候補へ入れにくい操作導線へ整理した。
