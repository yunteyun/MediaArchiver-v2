...
- フォルダ切替やサイドバー集計では、必要なファイルとタグだけをまとめて読むよう見直し、大きいライブラリでも一覧の切替待ちが伸びにくい形へ寄せた。
- 大きい登録フォルダのスキャンは、既存件数が多い場合に最初の件数カウントを省略できるようにし、開始時の待ち時間を短くした。
- `新規ファイル監視` は、変化が起きたサブフォルダだけを優先して見直すようにし、更新のたびに登録フォルダ全体を触りにくくした。
- 一覧で今見えているカードにサムネイルが無い場合は、表示中のものから静かに先回り生成するようにし、スクロール直後の見た目を整えやすくした。
- `build:electron` は毎回 `dist` / `dist-electron` を作り直してから梱包するようにし、古いアセット名を拾って初回だけ失敗しやすい状態を解消した。
- 日付グループ表示に「古い月も週ごとでまとめ続ける」切替を追加し、長い期間を週単位で追いたい使い方にも合わせやすくした。
- 週単位の日付グループ見出しは、同じ月内なら `第n週`、月またぎなら開始日週として表記し、まとまりの意味を読み取りやすくした。
- よく使う一覧の並び方を `新着順 / 週ごと / 月ごと / タイプ別` からワンタッチで呼べるようにし、グループ設定を切り替えやすくした。
- 一覧プリセットは検索欄と同じ列に常設プルダウンで置き、詳細設定を開かなくても見方を素早く切り替えられるようにした。
- 上段の `タイプ` と `一覧` の並び順を入れ替え、一覧プリセットの幅も表示内容に合わせて詰めて見た目を整えた。
- 検索語、検索対象、タイプ絞り込み、簡易評価フィルタはプロファイルごとに自動保存するようにし、再起動や切り替え後も前回の絞り込みへ戻りやすくした。
- 上段に `絞り込み解除` を追加し、保存される検索状態をワンタッチで初期化できるようにした。
- フィルタ欄から現在の見方をそのままスマートフォルダへ保存できるようにし、よく使う絞り込みを名前付きで残しやすくした。
- 保存済みスマートフォルダは上下に並び替えや折りたたみ表示に対応し、操作は編集メニューへまとめ、条件もタグ名を含む詳細ツールチップ付きのアイコンで見分けやすくした。

### Fixed
- ルート直下に残っていた開発中の不要ファイルや私的メモを整理し、配布や保守の判断をしやすくした。
- スキャン完了後の存在確認で同じファイルを何度もディスク確認しないようにし、大量ファイル環境で完了待ちが重くなりにくいようにした。
- 動画サムネイル生成が続いている間に、再生時間取得やメタデータ確認まで同じ待ち列で詰まりやすい問題を改善した。
