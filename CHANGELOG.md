...
## [Unreleased]
### Changed
- 一覧表示モードに `詳細表示` を追加し、横長カードでファイル名・タグ・基本情報をまとめて把握しやすい表示プリセットを選べるようにした。
- 詳細表示モードのカード構造を `サムネイル左 / 情報右` の横並びへ調整し、一覧上で情報を同時に読み取りやすくした。
- 詳細表示モードのサムネイル領域を正方形に統一し、横並びカードでもプレビューの見え方を安定させた。
- 詳細表示モードの情報欄を `2段バッジ + 複数行タグ` で整理し、一覧上で更新日・拡張子・フォルダ情報とタグを同時に読み取りやすくした。
- 表示モード名を `WhiteBrowser風` から `詳細表示` に変更し、機能意図が分かる名称へ統一した。
- 未解決不具合・要望の棚卸しを実施し、右サイドバーのタグ編集モード安全化と FileCard 中央寄せ時の余白最適化を直近タスクとして整理した。
- FileCard 一覧の伸縮表示で行配置を左揃えに統一し、上段だけ中央寄せになる見た目のばらつきを解消した。
- 詳細表示のカード幅計算に余白の再配分を追加し、右側に余りやすい空白を抑えてウィンドウ幅に追従しやすくした。
- 右サイドバーのタグ操作を編集モード式へ変更し、通常時は表示のみ・編集時のみ追加/削除できるようにして誤操作を防ぎやすくした。
