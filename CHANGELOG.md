...
- 動画サムネイル生成と動画/音声メタデータ取得も別プロセス化し、スキャンや再生成時の重い `ffmpeg` / `ffprobe` 処理をまとめて分離した。
- スキャン中の新規/更新ファイルを現在の一覧へ段階反映するようにし、完了まで待たずに追加結果を確認しやすくした。
- スキャン停止時は進行中の動画プレビューフレーム生成と後処理を早めに打ち切るようにし、停止操作後の待ち時間を短縮した。

### Fixed
- スキャン中の一覧自動更新を完了時反映へ見直し、サムネイル表示のもたつきや右クリックしづらくなる退行を修正。
