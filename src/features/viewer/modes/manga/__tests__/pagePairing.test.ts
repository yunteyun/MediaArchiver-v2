...
describe('stepPage / single モード', () => {
    it('next: +1', () => { expect(stepPage(3, 'next', 10, singleMode)).toBe(4); });
    it('prev: -1', () => { expect(stepPage(3, 'prev', 10, singleMode)).toBe(2); });
    it('先頭で prev: 0 に留まる', () => { expect(stepPage(0, 'prev', 10, singleMode)).toBe(0); });
    it('末尾で next: 末尾に留まる', () => { expect(stepPage(9, 'next', 10, singleMode)).toBe(9); });
});

describe('stepPage / spread + firstPageSingle=true', () => {
    it('index=0(表紙) → next → 1', () => { expect(stepPage(0, 'next', 10, spreadRtlSingle)).toBe(1); });
    it('index=1 → next → 3', () => { expect(stepPage(1, 'next', 10, spreadRtlSingle)).toBe(3); });
    it('index=3 → next → 5', () => { expect(stepPage(3, 'next', 10, spreadRtlSingle)).toBe(5); });
    it('index=1 → prev → 0（表紙へ）', () => { expect(stepPage(1, 'prev', 10, spreadRtlSingle)).toBe(0); });
    it('index=3 → prev → 1', () => { expect(stepPage(3, 'prev', 10, spreadRtlSingle)).toBe(1); });
    it('index=0 → prev → 0（先頭に留まる）', () => { expect(stepPage(0, 'prev', 10, spreadRtlSingle)).toBe(0); });
    it('最終ペア: next → 移動しない', () => { expect(stepPage(3, 'next', 5, spreadRtlSingle)).toBe(3); });
    it('totalCount=2, index=0 → next → 1', () => { expect(stepPage(0, 'next', 2, spreadRtlSingle)).toBe(1); });
    it('totalCount=1, index=0 → next → 0', () => { expect(stepPage(0, 'next', 1, spreadRtlSingle)).toBe(0); });
});

describe('stepPage / spread + firstPageSingle=false', () => {
    it('index=0 → next → 2', () => { expect(stepPage(0, 'next', 10, spreadRtlNoSingle)).toBe(2); });
    it('index=2 → next → 4', () => { expect(stepPage(2, 'next', 10, spreadRtlNoSingle)).toBe(4); });
    it('index=0 → prev → 0（先頭に留まる）', () => { expect(stepPage(0, 'prev', 10, spreadRtlNoSingle)).toBe(0); });
    it('index=2 → prev → 0', () => { expect(stepPage(2, 'prev', 10, spreadRtlNoSingle)).toBe(0); });
    it('最終ペアで next → 移動しない', () => { expect(stepPage(4, 'next', 5, spreadRtlNoSingle)).toBe(4); });
});
