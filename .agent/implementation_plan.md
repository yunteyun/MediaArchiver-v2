...
- 相対パス保存化を行うかどうかの設計判断（別タスク候補）
- `thumbnail_path` / `preview_frames` の絶対パス前提コスト（文字列置換ベース）の継続評価
- 旧構造データが必要になった場合の fallback 実地確認（任意）
...
- 判定方針:
  - 上記4点に問題がなければ Step 5 は「完了」として扱う（以後は回帰監視）
...
1. 回帰監視（リリース前の最小確認）
   - 再スキャン / 再生成 / プロファイル切替 / cleanup 後確認を短時間で実施する
2. 別タスク判断
   - 相対パス化を切り出すかどうかを優先度ベースで判断する
3. 条件付き確認
   - 旧構造データが用意できた場合のみ fallback 実地確認を再実施する
4. ドキュメント反映
   - 状態変更時のみ `ROADMAP.md` / `CHANGELOG.md` を更新する
