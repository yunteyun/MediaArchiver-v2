...
- **Current Focus**: Issue #33 修正（サブフォルダ選択的登録）
- **Current Status**: サブフォルダ一括登録でチェックを外したものも登録されてしまうバグを修正。ダイアログ内にチェックリストを展開表示する方式に変更。
- **Recent Achievements**:
  - **Issue #33 修正**:
    - 「サブフォルダも登録する」ONにするとサブフォルダ一覧がチェックリストで展開表示される
    - チェックを外したサブフォルダは登録されない
    - 全選択/解除ボタン付き、indeterminate状態対応
  - **v1.15.0**:
    - フォルダ登録に「サブフォルダも登録する」オプション追加（Issue #29 Close）
    - サイドバーで未登録子フォルダの右クリックメニュー追加
    - リネーム用クイック挿入の設定消失バグ修正（Issue #28 Close）
    - グループ表示の昇降順設定をグループ自体の並び順に反映（Issue #31 Close）
    - フィルター・表示設定バーのUI改善（ソート分離・検索オプション折りたたみ）（Issue #30 Close）
  - **v1.14.0**:
