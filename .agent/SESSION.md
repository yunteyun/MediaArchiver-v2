...
- **Current Focus**: Issue #29 対応（フォルダ登録・子フォルダ管理の改善）
- **Current Status**: フォルダ登録ダイアログへのサブフォルダ一括登録オプション追加と、未登録子フォルダへの右クリックメニュー対応を実装。
- **Recent Achievements**:
  - **#29 対応**: フォルダ登録時「サブフォルダも登録する」オプション追加、未登録サブフォルダの右クリックメニュー追加
  - **#28 修正**: リネーム用クイック挿入の設定がアップデート後に消える問題を修正（SQLite への即時保存を追加）
  - **v1.14.0**:
