...
- **Current Focus**: Issue #35・#36 修正（サブフォルダスキャン動作・ツリー仮想ノード表示）
- **Current Status**: 完了。フォルダ登録ダイアログを3択ラジオボタンに刷新し、「直下のみスキャン」モードを追加。
- **Recent Achievements**:
  - **Issue #35・#36 修正**:
    - フォルダ登録ダイアログの「サブフォルダの扱い」を3択ラジオボタンに統合
    - 「直下のファイルのみスキャンする」選択時にサブフォルダを再帰スキャンしないよう制御
    - サブフォルダがスキャンされないため、ツリーに仮想ノードが表示されなくなった
    - FolderScanSettings に shallowScan フラグを追加し、DB/IPC/型定義を全層対応
  - **Issue #33 修正**:
